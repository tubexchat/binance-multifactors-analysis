...
class ICAnalysis:
    ...
    def calculate_forward_returns(
        ...
        for period in periods:
            ...
            returns_df[f"forward_return_{period}"] = (
                future_prices - close_prices
            ) / close_prices
    ...
    def calculate_ic(
        ...
        try:
            ...
            merged_data = pd.merge(
                factor_data, return_data, on="timestamp", how="inner"
            )
            ...
            for return_col in return_columns:
                ...
                for factor_col in factor_columns:
                    ...
                    # 提取因子和收益率数据
                    factor_series = merged_data[factor_col]
                    return_series = merged_data[return_col]

                    # 确保数据是数值类型并移除NaN/无穷值
                    try:
                        factor_values = pd.to_numeric(factor_series, errors="coerce")
                        return_values = pd.to_numeric(return_series, errors="coerce")

                        # 创建布尔掩码，移除NaN和无穷值
                        valid_mask = (
                            pd.notna(factor_values)
                            & pd.notna(return_values)
                        ...
                        # 确保数据是1D数组且长度一致
                        factor_clean = np.asarray(factor_clean).flatten()
                        return_clean = np.asarray(return_clean).flatten()

                        if len(factor_clean) != len(return_clean):
                            logger.warning(
                                f"因子 {factor_col} 和收益率 {return_col} 长度不匹配: {len(factor_clean)} vs {len(return_clean)}"
                            )
                            ic_row[factor_col] = np.nan
                            ic_row[f"{factor_col}_pvalue"] = np.nan
                            continue

                        if len(factor_clean) < 10:  # 最少需要10个有效数据点
                            ic_row[factor_col] = np.nan
                            ic_row[f"{factor_col}_pvalue"] = np.nan
                            continue

                        # 检查数据是否有足够的变异性
                        if np.std(factor_clean) == 0 or np.std(return_clean) == 0:
                            ic_row[factor_col] = np.nan
                            ic_row[f"{factor_col}_pvalue"] = np.nan
                            continue
                        ...
                        # 计算相关性
                        if method == "pearson":
                            corr, p_value = stats.pearsonr(factor_clean, return_clean)
                        elif method == "spearman":
                            corr, p_value = stats.spearmanr(factor_clean, return_clean)
                        elif method == "kendall":
                            corr, p_value = stats.kendalltau(factor_clean, return_clean)
                        else:
                        ...
                        logger.warning(
                            f"计算 {factor_col} 与 {return_col} 的IC时出错: {e}"
                        )
                        ic_row[factor_col] = np.nan
                        ic_row[f"{factor_col}_pvalue"] = np.nan
                        ...
                        try:
                            if method == "pearson":
                                corr, _ = stats.pearsonr(
                                    valid_data[factor_col], valid_data[return_col]
                                )
                            elif method == "spearman":
    ...
    def calculate_ic_statistics(self, ic_data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            factor_columns = [
                ...
                if col not in ["return_period", "timestamp"]
                and not col.endswith("_pvalue")
            ]
    ...
    def rank_factors_by_ic(
        self, ic_stats: pd.DataFrame, ranking_method: str = "abs_mean"
    ) -> pd.DataFrame:
        """
        根据IC指标对因子进行排名
                ...
                for col in row.index:
                    if col.endswith(f"_{ranking_method}") and not pd.isna(row[col]):
                        ...
                        factor_scores[factor_name] = (
                            abs(row[col]) if ranking_method != "abs_mean" else row[col]
                        )
    ...
    def filter_effective_factors(
        self, ic_stats: pd.DataFrame, threshold: float = None
    ) -> List[str]:
        """
        筛选有效因子
        ...
        try:
            ...
            logger.info(
                f"筛选出 {len(effective_factors)} 个有效因子（阈值: {threshold}）"
            )
    ...
    def analyze_factor_stability(
        self, rolling_ic_results: Dict[str, pd.DataFrame]
    ) -> pd.DataFrame:
        """
        分析因子稳定性
        ...
        try:
            ...
            for return_period, rolling_ic_df in rolling_ic_results.items():
                factor_columns = [
                    col for col in rolling_ic_df.columns if col != "timestamp"
                ]
                ...
                for factor_col in factor_columns:
                    ...
                    ic_significant_ratio = (
                        abs(ic_series) > self.ic_config.IC_THRESHOLD
                    ).mean()
                    ...
                    stability_results.append(
                        {
                            ...
                            "stability_score": ic_significant_ratio
                            * (
                                1 - abs(max_drawdown) / abs(cumulative_ic.iloc[-1])
                                if cumulative_ic.iloc[-1] != 0
                                else 0
                            ),
                        }
                    )
    ...
    def generate_ic_report(
        self, symbol: str, factor_data: pd.DataFrame, price_data: pd.DataFrame
    ) -> Dict[str, pd.DataFrame]:
        """
        生成完整的IC分析报告
